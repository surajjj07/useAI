...
const BlogTitles = () => {
  ...
  const formHandler = async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Generate a blog title for the keyword ${input} in category ${selectedcategory}`
      
      const { data } = await axios.post('/api/ai/generate-blog-title', {prompt}, {withCredentials:true})
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={formHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {blogCategories.map((item) => (<span onClick={() => setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${item === selectedcategory ? 'bg-purple-50 text-purple-700' : 'text-gray-600 border-gray-300 '}`} key={item}>{item}</span>))}
        </div>
        <br />
